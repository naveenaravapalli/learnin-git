...
struct list *print_nth_node_from_last(struct list *);
...
int main()
{
	...
	while(1)
	{
		printf("1.create_node  2.delete_node  3.reverse_list  4.display  5.loop_create  6.loop_detect_break  7.print_Nth_node  8.print_Nth_node_from_last  9.exit\n");
		...
		switch(choice)
		{
			...
			case 7: head = print_nth_node(head);
				break;
			case 8: head = print_nth_node_from_last(head);
				break;
			case 9: exit(0);
...
struct list *print_nth_node_from_last(struct list *ptr)
{
	if(ptr == NULL)
	{
		printf("list is empty\n");
	...
	int num,cnt = 0,node,cnt1 = 0;
	printf("enter the node number to display data from last : ");
	scanf("%d",&num);
	while(temp)
	{
		cnt++;
